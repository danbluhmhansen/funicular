...
public static class JsonDocumentConverter
{
    public static object Convert(object obj)
    {
        ...
        else if (obj is JsonDocument jsonDocument)
        {
            ...
            ConvertValue(jsonDocument.RootElement, setValue, new HashSet<object>());
        }
    ...
    private static object GetValue(JsonElement element)
    {
        switch (element.ValueKind)
        {
            ...
            case JsonValueKind.Number:
                ...
                if (
                    element.TryGetSingle(out var floatValue)
                    && !(float.IsInfinity(floatValue) || float.IsNegativeInfinity(floatValue))
                )
                    return floatValue;

                if (
                    element.TryGetDouble(out var doubleValue)
                    && !(double.IsInfinity(doubleValue) || double.IsNegativeInfinity(doubleValue))
                )
                    return doubleValue;
