...
using Funicular.Server.Models.Authorization;
...
public class AuthorizationController : ControllerBase
{
    ...
    public AuthorizationController(
        ...
        SignInManager<FunicularUser> signInManager,
        UserManager<FunicularUser> userManager
    )
    {
    ...
    public async Task<IActionResult> Authorize()
    {
            ...
            if (!string.IsNullOrEmpty(request.IdentityProvider))
            {
                ...
                var properties = signInManager.ConfigureExternalAuthenticationProperties(
                    ...
                    redirectUrl: Url.Action(
                        ...
                        "Account",
                        new { ReturnUrl = Request.PathBase + Request.Path + QueryString.Create(parameters) }
                    )
                );
        ...
        switch (await applicationManager.GetConsentTypeAsync(application))
        {
            ...
            case ConsentTypes.Explicit when authorizations.Any() && !request.HasPrompt(Prompts.Consent):
                ...
                identity
                    .AddClaim(Claims.Subject, await userManager.GetUserIdAsync(user))
                    .AddClaim(Claims.Email, await userManager.GetEmailAsync(user))
                    .AddClaim(Claims.Name, await userManager.GetUserNameAsync(user))
                    .AddClaims(Claims.Role, (await userManager.GetRolesAsync(user)).ToImmutableArray());
            ...
            default:
                return Ok(
                    new Authorize(await applicationManager.GetLocalizedDisplayNameAsync(application), request.Scope)
                );
        }
    }
        ...
        if (result.Succeeded)
        {
            ...
            return Ok(
                new Verify(
                    ...
                    string.Join(" ", result.Principal.GetScopes()),
                    request.UserCode
                )
            );
        }
