...
#[typed_path("/partials/gear-kinds-table/:game_slug")]
...
pub(crate) async fn get(Path { game_slug }: Path, State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let kinds = sqlx::query!(
        "
        SELECT gear_kind.name, gear_kind.slug FROM gear_kind
    ...
    Html(markup::new! {
        table #"gear-kinds-table"["x-data"="{ toggle: false }",class="w-full"] {
            ...
            tbody {
                @for kind in kinds.iter() {
                    tr[class="bg-white border-b last:border-0 dark:bg-slate-800 dark:border-slate-700"] {
                        ...
                        td[class="p-3 text-center"] {
                            a[
                                href=routes::games::game::gears::Path::new(
                                    game_slug.clone(),
                                    // TODO: avoid clone?
