...
SELECT
    ...
    SUM(
        "rule_num"."value" * COALESCE("actor_trait"."amount", 0) + "rule_num"."value" * COALESCE("gear_trait"."amount", 0)
    ) AS "value"
FROM
    "public"."gear"
    ...
    JOIN "public"."rule_num" ON "rule_num"."skill_id" = ANY(
        ARRAY ["gear_skill"."skill_id", "sub_skill"."sup_id"]
    )
    JOIN "public"."trait" ON "trait"."id" = "rule_num"."trait_id"
    LEFT JOIN "public"."gear_trait" ON "gear_trait"."gear_id" = "gear"."id"
    AND "gear_trait"."trait_id" = "trait"."id"
    LEFT JOIN "public"."actor_trait" ON "actor_trait"."actor_id" = "actor"."id"
    AND "actor_trait"."trait_id" = "trait"."id"
GROUP BY
    "gear_kind"."game_id",
    "actor_gear"."actor_id",
    "gear"."id",
    COALESCE("sub_skill"."sub_id", "gear_skill"."skill_id")
ORDER BY
    1,
    2,
    3,
